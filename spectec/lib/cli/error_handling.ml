...
let guard ?(suppress_trace = false) ~color ~on_ok f =
  ...
  if not (Spectec.Diagnostic.Bag.is_empty bag) then
    Printf.eprintf "%s\n%!"
      (Spectec.Diagnostic.Render.render_bag ~show_trace:(not suppress_trace)
         ~ansi bag);
  match result with Ok v -> on_ok v | Error _ -> exit 1

let guard_unit ?suppress_trace ~color f =
  guard ?suppress_trace ~color ~on_ok:ignore f
