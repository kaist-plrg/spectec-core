(* Precedence mirrors the parser, so output re-parses to the same tree. *)
...
let rec string_of_expr ~prec (v : Value.t) : string =
  ...
  (* Leaves may carry different synids; match on the atom, not the synid. *)
  ...
  | "expr", [ "?"; ":" ], [ c; t; e ] ->
      paren (prec > 0)
        (string_of_expr ~prec:1 c ^ " ? " ^ string_of_expr ~prec:0 t ^ " : "
  ...
  | "expr", [ "+" ], [ l; r ] ->
      paren (prec > 3)
        (string_of_expr ~prec:3 l ^ " + " ^ string_of_expr ~prec:4 r)
  | "expr", [ "!" ], [ e ] -> paren (prec > 4) ("!" ^ string_of_expr ~prec:4 e)
  | "expr", [ "("; ")" ], [ f; a ] ->
      paren (prec > 5)
        (string_of_expr ~prec:5 f ^ "(" ^ string_of_expr ~prec:0 a ^ ")")
...
and string_of_command (v : Value.t) : string =
  ...
  | ( "command",
      [ "REC"; "("; ")"; "->"; "{"; "}" ],
      [ fname; t_arg; x; t_ret; body ] ) ->
      Printf.sprintf "rec %s (%s %s) -> %s { %s }" (text_of_id fname)
        (string_of_type t_arg) (text_of_id x) (string_of_type t_ret)
        (string_of_expr ~prec:0 body)
