...
let info lexbuf : Surface.Info.t =
  let f = filename () in
...
rule tokenize = parse
  ...
  | '"'
      { let str, end_info = (string lexbuf) in
        STRING_LITERAL ({tags=Surface.Info.merge (info lexbuf) end_info; str}) }
  | whitespace
      { tokenize lexbuf }
  | '#'
  ...
  | "@pragma"
      { PRAGMA (info lexbuf) }
  | hex_number as n
      { NUMBER (parse_int n (info lexbuf), n) }
  | dec_number as n
      { NUMBER (parse_int (strip_prefix n) (info lexbuf), n) }
  | oct_number as n
      { NUMBER (parse_int n (info lexbuf), n) }
  | bin_number as n
      { NUMBER (parse_int n (info lexbuf), n) }
...
let rec lexer (lexbuf:lexbuf): token = 
   match !lexer_state with
    ...
    | SRangle info1 -> 
      ...
      | R_ANGLE info2 when Surface.Info.follows info1 info2 -> 
        lexer_state := SRegular;
        R_ANGLE_SHIFT info2
      | PRAGMA _ as token ->
        lexer_state := SPragma;
