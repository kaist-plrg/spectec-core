...
let find_var (name : Ds.Var.t) ((genv, lenv, sto) : Env.t * Env.t * Sto.t) :
    Type.t * Value.t =
  let addr = try Env.find name lenv with Not_found -> Env.find name genv in
  Sto.find addr sto

let find_var_global (name : Ds.Var.t) ((genv, _, sto) : Env.t * Env.t * Sto.t) :
    Type.t * Value.t =
...
let update_value (name : Ds.Var.t) (value : Value.t)
    ((genv, lenv, sto) : Env.t * Env.t * Sto.t) =
