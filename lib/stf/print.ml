...
let print_option fmt printer = function
  | Some s -> printer fmt s
  | None -> ()
...
let print_stmt fmt = function
  ...
  | Expect (port, expect_opt) ->
      F.fprintf fmt "expect %a %a"
        print_port port
        (F.pp_print_option print_expect) expect_opt
  | Packet (port, packet) ->
  ...
  | Add (name, int_opt, mtchs, action, id_opt) ->
      F.fprintf fmt "add %a %a %a %a %a"
        print_name name
        (F.pp_print_option print_int) int_opt
        (F.pp_print_list
           ~pp_sep:(fun fmt () -> F.fprintf fmt " ")
           print_mtch)
        mtchs print_action action
        (F.pp_print_option print_id) id_opt
  ...
  | CheckCounter (id, id_or_index, (ctr, cond, number)) ->
      F.fprintf fmt "check_counter %a(%a) %a %a %a"
        print_id id print_id_or_index id_or_index
        (F.pp_print_option print_ctr) ctr
        print_cond cond print_number number
