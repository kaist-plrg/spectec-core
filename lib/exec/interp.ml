...
module Make (Arch : ARCH) : INTERP = struct
  ...
  and interp_str (ctx : Ctx.t) (t : text) : Ctx.t * Value.t = (ctx, StrV t.it)
  ...
  and interp_num (ctx : Ctx.t) (value : Bigint.t)
    ...
    let value =
      match encoding with
      | Some (width, signed) ->
          if signed then Value.FIntV (width, value)
          else Value.FBitV (width, value)
      | None -> IntV value
    in
    (ctx, value)
