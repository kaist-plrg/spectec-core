...
let check_lctk (expr : Il.Ast.expr) : unit res =
  check
...
let check_ctk (expr : Il.Ast.expr) : unit res =
...
let rec ctk_expr (cursor : Ctx.cursor) (ctx : Ctx.t) (expr : Il.Ast.expr') :
  ...
  | MaskE _ | RangeE _ | SelectE _ | ArrAccE _ -> Ok DYN
...
and ctk_var_expr (cursor : Ctx.cursor) (ctx : Ctx.t) (var : Il.Ast.var) :
    Ctk.t res =
...
and ctk_seq_expr (exprs : Il.Ast.expr list) : Ctk.t res =
  let ctks = List.map (fun expr -> Il.Ast.(expr.note.ctk)) exprs in
  let ctk = Ctk.joins ctks in
  Ok ctk

and ctk_record_expr (fields : (Il.Ast.member * Il.Ast.expr) list) : Ctk.t res =
  let exprs = List.map snd fields in
  let ctks = List.map (fun expr -> Il.Ast.(expr.note.ctk)) exprs in
  let ctk = Ctk.joins ctks in
  Ok ctk

and ctk_unop_expr (_unop : Il.Ast.unop) (expr : Il.Ast.expr) : Ctk.t res =
  let ctk = expr.note.ctk in
  Ok ctk

and ctk_binop_expr (_binop : Il.Ast.binop) (expr_l : Il.Ast.expr)
    (expr_r : Il.Ast.expr) : Ctk.t res =
...
and ctk_ternop_expr (expr_cond : Il.Ast.expr) (expr_then : Il.Ast.expr)
    (expr_else : Il.Ast.expr) : Ctk.t res =
  let ctks = [ expr_cond.note.ctk; expr_then.note.ctk; expr_else.note.ctk ] in
  let ctk = Ctk.joins ctks in
  Ok ctk

and ctk_cast_expr (_typ : Il.Ast.typ) (expr : Il.Ast.expr) : Ctk.t res =
  let ctk = expr.note.ctk in
  Ok ctk

and ctk_bitstring_acc_expr (expr_base : Il.Ast.expr) (_value_lo : Il.Ast.value)
    (_value_hi : Il.Ast.value) : Ctk.t res =
  let ctk = expr_base.note.ctk in
  Ok ctk

and ctk_expr_acc_expr (expr_base : Il.Ast.expr) (member : Il.Ast.member) :
    Ctk.t res =
  ...
  | LCTK -> (
      match typ_base with
      | Types.StackT _ when member.it = "size" -> Ok LCTK
      | _ -> Ok DYN)
  | CTK | DYN -> Ok DYN

and ctk_call_method_expr (expr_base : Il.Ast.expr) (member : Il.Ast.member)
    (targs : Il.Ast.targ list) (args : Il.Ast.arg list) : Ctk.t res =
  let typ_base = expr_base.note.typ in
  ...
  | "minSizeInBits" | "minSizeInBytes" | "maxSizeInBits" | "maxSizeInBytes" ->
      let* _ =
        check
          (targs = [] && args = [])
          (Format.asprintf
             "(ctk_call_method_expr) %s does not take type arguments or \
              arguments"
             member.it)
      in
...
and ctk_call_type_expr (typ : Il.Ast.typ) (member : Il.Ast.member) : Ctk.t res =
  match member.it with
  | "minSizeInBits" | "minSizeInBytes" | "maxSizeInBits" | "maxSizeInBytes" ->
...
and eval_record_expr (cursor : Ctx.cursor) (ctx : Ctx.t)
    (fields : (Il.Ast.member * Il.Ast.expr) list) : Value.t res =
...
and eval_binop_expr (cursor : Ctx.cursor) (ctx : Ctx.t) (binop : Il.Ast.binop)
    (expr_l : Il.Ast.expr) (expr_r : Il.Ast.expr) : Value.t res =
...
and eval_ternop_expr (cursor : Ctx.cursor) (ctx : Ctx.t)
    ...
    (expr_else : Il.Ast.expr) : Value.t res =
...
and eval_bitstring_acc_expr (cursor : Ctx.cursor) (ctx : Ctx.t)
    ...
    (value_hi : Il.Ast.value) : Value.t res =
...
and eval_expr_acc_expr (cursor : Ctx.cursor) (ctx : Ctx.t)
    (expr_base : Il.Ast.expr) (member : Il.Ast.member) : Value.t res =
  let* value_base = eval_expr cursor ctx expr_base in
  let value =
...
and eval_call_method_expr (_cursor : Ctx.cursor) (_ctx : Ctx.t)
    ...
    (targs : Il.Ast.targ list) (args : Il.Ast.arg list) : Value.t res =
  let value =
    match member.it with
    | "minSizeInBits" | "minSizeInBytes" | "maxSizeInBits" | "maxSizeInBytes" ->
        assert (targs = [] && args = []);
...
and eval_call_type_expr (_cursor : Ctx.cursor) (_ctx : Ctx.t) (typ : Il.Ast.typ)
    (member : Il.Ast.member) : Value.t res =
  let value =
    match member.it with
    | "minSizeInBits" | "minSizeInBytes" | "maxSizeInBits" | "maxSizeInBytes" ->
