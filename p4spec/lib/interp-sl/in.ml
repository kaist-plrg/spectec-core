...
let in_opt (do_in : Ctx.t -> 'a -> value) (typ : typ) (ctx : Ctx.t)
    (opt : 'a option) : value =
  let value_opt = Option.map (do_in ctx) opt in
  let value =
...
let in_list (do_in : Ctx.t -> 'a -> value) (typ : typ) (ctx : Ctx.t)
    (lst : 'a list) : value =
...
let in_pair (do_in_a : Ctx.t -> 'a -> value) (do_in_b : Ctx.t -> 'b -> value)
    (typ_a : typ) (typ_b : typ) (ctx : Ctx.t) ((a, b) : 'a * 'b) : value =
  let value_a = do_in_a ctx a in
  let value_b = do_in_b ctx b in
  let value =
    let vid = Dep.Graph.fresh () in
...
let in_num (ctx : Ctx.t) (num : P4.num) : value =
  let mixop, values =
    ...
    | i, Some (width, signed) ->
        ...
        in
        Ctx.add_node ~taint:true ctx value_width;
        let value_int =
          let vid = Dep.Graph.fresh () in
...
and in_members (ctx : Ctx.t) (members : P4.member list) : value =
  in_list in_member (in_typ_var "member" $ no_region) ctx members
...
let rec in_typ (ctx : Ctx.t) (typ : P4.typ) : value =
  let mixop, values =
    ...
    | FIntT expr ->
        ...
        let value_expr = in_expr ctx expr in
        (mixop, [ value_expr ])
    | FBitT expr ->
    ...
    | StackT (typ, expr) ->
        ...
        let value_typ = in_typ ctx typ in
        let value_expr = in_expr ctx expr in
    ...
    | TupleT typs ->
        ...
        let value_typs = in_typs ctx typs in
        (mixop, [ value_typs ])
    | NameT var ->
        let mixop = [ [ in_atom "NameT" ]; [] ] in
        let value_var = in_var ctx var in
        (mixop, [ value_var ])
    | SpecT (var, targs) ->
        let mixop = [ [ in_atom "SpecT" ]; []; [] ] in
        let value_var = in_var ctx var in
        let value_targs = in_targs ctx targs in
...
and in_typs (ctx : Ctx.t) (typs : P4.typ list) : value =
  in_list in_typ (in_typ_var "type" $ no_region) ctx typs
...
and in_tparams (ctx : Ctx.t) (tparams : P4.tparam list) : value =
  in_list in_tparam (in_typ_var "tparam" $ no_region) ctx tparams
...
and in_param (ctx : Ctx.t) (param : P4.param) : value =
  ...
  let mixop = [ []; []; []; []; [] ] in
  let value_id = in_id ctx id in
  let value_dir = in_dir ctx dir in
  let value_typ = in_typ ctx typ in
  let value_expr_opt =
    in_opt in_expr (in_typ_var "expr" $ no_region) ctx expr_opt
  in
  let value =
    let vid = Dep.Graph.fresh () in
...
and in_params (ctx : Ctx.t) (params : P4.param list) : value =
  in_list in_param (in_typ_var "param" $ no_region) ctx params

(* Constructor parameters *)

and in_cparam (ctx : Ctx.t) (cparam : P4.cparam) : value =
...
and in_cparams (ctx : Ctx.t) (cparams : P4.cparam list) : value =
  in_list in_cparam (in_typ_var "cparam" $ no_region) ctx cparams

(* Type arguments *)

and in_targ (ctx : Ctx.t) (targ : P4.targ) : value =
  let mixop, values =
    match targ.it with
    | VoidT ->
        let mixop = [ [ in_atom "VoidT" ] ] in
    ...
    | StackT (typ, expr) ->
        let mixop = [ [ in_atom "StackT" ]; []; [] ] in
        let value_typ = in_targ ctx typ in
        let value_expr = in_expr ctx expr in
        (mixop, [ value_typ; value_expr ])
    | ListT typ ->
        let mixop = [ [ in_atom "ListT" ]; [] ] in
    ...
    | TupleT typs ->
        let mixop = [ [ in_atom "TupleT" ]; [] ] in
        let value_typs = in_targs ctx typs in
        (mixop, [ value_typs ])
    | NameT var ->
        let mixop = [ [ in_atom "NameT" ]; [] ] in
        let value_var = in_var ctx var in
...
and in_targs (ctx : Ctx.t) (targs : P4.targ list) : value =
  in_list in_targ (in_typ_var "targ" $ no_region) ctx targs
...
and in_args (ctx : Ctx.t) (args : P4.arg list) : value =
  in_list in_arg (in_typ_var "arg" $ no_region) ctx args
...
and in_expr (ctx : Ctx.t) (expr : P4.expr) : value =
  let mixop, values =
    ...
    | BoolE { boolean } ->
        ...
        let value_boolean = in_bool ctx boolean in
        (mixop, [ value_boolean ])
    | StrE { text } ->
        let mixop = [ [ in_atom "StrE" ]; [] ] in
        let value_text = in_text ctx text in
        (mixop, [ value_text ])
    | NumE { num } ->
        let mixop = [ [ in_atom "NumE" ]; [] ] in
        let value_num = in_num ctx num in
        (mixop, [ value_num ])
    | VarE { var } ->
    ...
    | SeqE { exprs } ->
        ...
        let value_exprs = in_exprs ctx exprs in
        (mixop, [ value_exprs ])
    | SeqDefaultE { exprs } ->
    ...
    | UnE { unop; expr } ->
        ...
        let value_unop = in_unop ctx unop in
        let value_expr = in_expr ctx expr in
        (mixop, [ value_unop; value_expr ])
    | BinE { binop; expr_l; expr_r } ->
        let mixop = [ [ in_atom "BinE" ]; []; []; [] ] in
        let value_binop = in_binop ctx binop in
        let value_expr_l = in_expr ctx expr_l in
        let value_expr_r = in_expr ctx expr_r in
        (mixop, [ value_binop; value_expr_l; value_expr_r ])
    | TernE { expr_cond; expr_then; expr_else } ->
        let mixop = [ [ in_atom "TernE" ]; []; []; [] ] in
        let value_expr_cond = in_expr ctx expr_cond in
        let value_expr_then = in_expr ctx expr_then in
        let value_expr_else = in_expr ctx expr_else in
    ...
    | CallFuncE { var_func; targs; args } ->
        ...
        let value_var_func = in_var ctx var_func in
        let value_targs = in_targs ctx targs in
        let value_args = in_args ctx args in
    ...
    | CallTypeE { var_typ; member; targs; args } ->
        ...
        let value_var_typ = in_var ctx var_typ in
        let value_member = in_member ctx member in
        let value_targs = in_targs ctx targs in
        let value_args = in_args ctx args in
    ...
    | InstE { var_inst; targs; args } ->
        ...
        let value_var_inst = in_var ctx var_inst in
        let value_targs = in_targs ctx targs in
        let value_args = in_args ctx args in
...
and in_exprs (ctx : Ctx.t) (exprs : P4.expr list) : value =
  in_list in_expr (in_typ_var "expr" $ no_region) ctx exprs
...
and in_keysets (ctx : Ctx.t) (keysets : P4.keyset list) : value =
  in_list in_keyset (in_typ_var "keyset" $ no_region) ctx keysets
...
and in_select_case (ctx : Ctx.t) (select_case : P4.select_case) : value =
  ...
  let mixop = [ []; []; [] ] in
  let value_keysets = in_keysets ctx keysets in
  let value_state_label = in_state_label ctx state_label in
  let value =
    let vid = Dep.Graph.fresh () in
...
and in_select_cases (ctx : Ctx.t) (select_cases : P4.select_case list) : value =
  in_list in_select_case (in_typ_var "selectcase" $ no_region) ctx select_cases
...
and in_stmt (ctx : Ctx.t) (stmt : P4.stmt) : value =
  let mixop, values =
    ...
    | SwitchS { expr_switch; cases } ->
        ...
        let value_expr_switch = in_expr ctx expr_switch in
        let value_cases = in_switch_cases ctx cases in
        (mixop, [ value_expr_switch; value_cases ])
    | IfS { expr_cond; stmt_then; stmt_else } ->
        let mixop = [ [ in_atom "IfS" ]; []; []; [] ] in
        let value_expr_cond = in_expr ctx expr_cond in
        let value_stmt_then = in_stmt ctx stmt_then in
        let value_stmt_else = in_stmt ctx stmt_else in
        (mixop, [ value_expr_cond; value_stmt_then; value_stmt_else ])
    | BlockS { block } ->
        let mixop = [ [ in_atom "BlockS" ]; [] ] in
        let value_block = in_block ctx block in
        (mixop, [ value_block ])
    | ExitS ->
    ...
    | RetS { expr_ret } ->
        ...
        let value_expr_ret =
          in_opt in_expr (in_typ_var "expr" $ no_region) ctx expr_ret
        in
    ...
    | TransS { expr_label } ->
        ...
        let value_expr_label = in_expr ctx expr_label in
        (mixop, [ value_expr_label ])
    | DeclS { decl } ->
        let mixop = [ [ in_atom "DeclS" ]; [] ] in
        let value_decl = in_decl ctx decl in
        (mixop, [ value_decl ])
  in
  let value =
    let vid = Dep.Graph.fresh () in
...
and in_stmts (ctx : Ctx.t) (stmts : P4.stmt list) : value =
  in_list in_stmt (in_typ_var "stmt" $ no_region) ctx stmts
...
and in_block (ctx : Ctx.t) (block : P4.block) : value =
  ...
  let value_stmts = in_stmts ctx stmts in
  let value =
    let vid = Dep.Graph.fresh () in
...
and in_switch_case (ctx : Ctx.t) (switch_case : P4.switch_case) : value =
  let mixop, values =
    ...
    | MatchC (switch_label, block) ->
        ...
        let value_switch_label = in_switch_label ctx switch_label in
        let value_block = in_block ctx block in
...
and in_switch_cases (ctx : Ctx.t) (switch_cases : P4.switch_case list) : value =
  in_list in_switch_case (in_typ_var "switchcase" $ no_region) ctx switch_cases
...
and in_decl (ctx : Ctx.t) (decl : P4.decl) : value =
  let mixop, values =
    ...
    | ConstD { id; typ; value; _ } ->
        ...
        let value_id = in_id ctx id in
        let value_typ = in_typ ctx typ in
        let value_value = in_expr ctx value in
    ...
    | VarD { id; typ; init; _ } ->
        ...
        let value_typ = in_typ ctx typ in
        let value_init =
          in_opt in_expr (in_typ_var "expr" $ no_region) ctx init
        in
        (mixop, [ value_id; value_typ; value_init ])
    | ErrD { members } ->
        let mixop = [ [ in_atom "ErrD" ]; [] ] in
        let value_members = in_members ctx members in
        (mixop, [ value_members ])
    | MatchKindD { members } ->
    ...
    | NewTypeD { id; typdef; _ } ->
        ...
        let value_id = in_id ctx id in
        let value_typdef = in_typdef ctx typdef in
    ...
    | TableD { id; table; _ } ->
        ...
        let value_id = in_id ctx id in
        let value_table = in_table ctx table in
    ...
    | ControlD { id; params; cparams; locals; body; _ } ->
        ...
        let value_cparams = in_cparams ctx cparams in
        let value_locals = in_decls ctx locals in
        let value_body = in_block ctx body in
        ( mixop,
    ...
    | FuncD { id; typ_ret; tparams; params; body; _ } ->
        ...
        let value_id = in_id ctx id in
        let value_typ_ret = in_typ ctx typ_ret in
        let value_tparams = in_tparams ctx tparams in
        let value_params = in_params ctx params in
        let value_body = in_block ctx body in
        ( mixop,
    ...
    | ExternObjectD { id; tparams; mthds; _ } ->
        ...
        let value_id = in_id ctx id in
        let value_tparams = in_tparams ctx tparams in
        let value_mthds = in_mthds ctx mthds in
...
and in_decls (ctx : Ctx.t) (decls : P4.decl list) : value =
  in_list in_decl (in_typ_var "decl" $ no_region) ctx decls
...
and in_table (ctx : Ctx.t) (table : P4.table) : value =
  in_list in_table_property (in_typ_var "tblprop" $ no_region) ctx table
...
and in_table_property (ctx : Ctx.t) (table_property : P4.table_property) : value
  ...
  let mixop, values =
    ...
    | KeyP table_keys ->
        ...
        let value_table_keys = in_table_keys ctx table_keys in
        (mixop, [ value_table_keys ])
    | ActionP table_actions ->
        let mixop = [ [ in_atom "ActionP" ]; [] ] in
        let value_table_actions = in_table_actions ctx table_actions in
        (mixop, [ value_table_actions ])
    | EntryP table_entries ->
        let mixop = [ [ in_atom "EntryP" ]; [] ] in
        let value_table_entries = in_table_entries ctx table_entries in
        (mixop, [ value_table_entries ])
    | DefaultP table_default ->
        let mixop = [ [ in_atom "DefaultP" ]; [] ] in
        let value_table_default = in_table_default ctx table_default in
        (mixop, [ value_table_default ])
    | CustomP table_custom ->
        let mixop = [ [ in_atom "CustomP" ]; [] ] in
        let value_table_custom = in_table_custom ctx table_custom in
...
and in_table_key (ctx : Ctx.t) (table_key : P4.table_key) : value =
  ...
  let mixop = [ []; []; [] ] in
  let value_expr = in_expr ctx expr in
  let value_match_kind = in_match_kind ctx match_kind in
  let value =
    let vid = Dep.Graph.fresh () in
...
and in_table_keys (ctx : Ctx.t) (table_keys : P4.table_keys) : value =
  in_list in_table_key (in_typ_var "tblkey" $ no_region) ctx table_keys.it
...
and in_table_entry (ctx : Ctx.t) (table_entry : P4.table_entry) : value =
  ...
  let mixop = [ []; []; []; []; [] ] in
  let value_table_entry_const = in_bool ctx table_entry_const in
  let value_keysets = in_keysets ctx keysets in
  let value_table_action = in_table_action ctx table_action in
  let value_expr_opt =
    in_opt in_expr (in_typ_var "expr" $ no_region) ctx expr_opt
  in
  let value =
...
and in_table_entries (ctx : Ctx.t) (table_entries : P4.table_entries) : value =
  ...
  let mixop = [ []; []; [] ] in
  let value_table_entries_const = in_bool ctx table_entries_const in
  let value_table_entries =
    in_list in_table_entry (in_typ_var "tblentry" $ no_region) ctx table_entries
  in
  let value =
    let vid = Dep.Graph.fresh () in
...
and in_table_default (ctx : Ctx.t) (table_default : P4.table_default) : value =
  ...
  let mixop = [ []; []; [] ] in
  let value_table_default_const = in_bool ctx table_default_const in
  let value_table_action = in_table_action ctx table_action in
  let value =
    let vid = Dep.Graph.fresh () in
...
and in_table_custom (ctx : Ctx.t) (table_custom : P4.table_custom) : value =
  ...
  let mixop = [ []; []; []; [] ] in
  let value_table_custom_const = in_bool ctx table_custom_const in
  let value_member = in_member ctx member in
  let value_expr = in_expr ctx expr in
  let value =
    let vid = Dep.Graph.fresh () in
...
and in_mthds (ctx : Ctx.t) (mthds : P4.mthd list) : value =
  in_list in_mthd (in_typ_var "mthd" $ no_region) ctx mthds
