...
let find_typdef_opt (ctx : t) (tid : TId.t phrase) : TypeDef.t option =
  TDEnv.find_opt tid.it ctx.tdenv
...
let bound_typdef (ctx : t) (tid : TId.t phrase) : bool =
  find_typdef_opt ctx tid |> Option.is_some

(* Finders for meta-variables *)

let find_metavar_opt (ctx : t) (tid : TId.t phrase) : Type.t option =
  TDEnv.find_opt tid.it ctx.menv
...
let find_rel_opt (ctx : t) (rid : RId.t phrase) : nottyp option =
  REnv.find_opt rid.it ctx.renv |> Option.map fst
...
let find_rules_opt (ctx : t) (rid : RId.t phrase) : Il.Ast.rule list option =
  REnv.find_opt rid.it ctx.renv |> Option.map snd
...
let find_dec_opt (ctx : t) (fid : FId.t phrase) :
    (tparam list * param list * plaintyp) option =
  FEnv.find_opt fid.it ctx.fenv
...
let find_clauses_opt (ctx : t) (fid : FId.t phrase) : Il.Ast.clause list option
    =
  FEnv.find_opt fid.it ctx.fenv
  |> Option.map (fun (_, _, _, clauses) -> clauses)
...
let update_typdef (ctx : t) (tid : TId.t phrase) (td : TypeDef.t) : t =
  if not (bound_typdef ctx tid) then error tid.at "undefined type";
  let tdenv = TDEnv.add tid.it td ctx.tdenv in
  { ctx with tdenv }
