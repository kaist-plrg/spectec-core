...
let in_typ_var (s : string) : typ' = VarT (s $ no_region, [])
...
let in_pair (do_in_a : 'a -> value) (do_in_b : 'b -> value) (typ_a : typ)
    (typ_b : typ) ((a, b) : 'a * 'b) : value =
...
and in_members (members : P4.member list) : value =
  in_list in_member (in_typ_var "member" $ no_region) members
...
let rec in_typ (typ : P4.typ) : value =
  let mixop, values =
    ...
    | TupleT typs ->
        ...
        let value_typs = in_typs typs in
        (mixop, [ value_typs ])
    | NameT var ->
    ...
    | SpecT (var, targs) ->
        ...
        let value_var = in_var var in
        let value_targs = in_targs targs in
...
and in_typs (typs : P4.typ list) : value =
  in_list in_typ (in_typ_var "type" $ no_region) typs
...
and in_tparams (tparams : P4.tparam list) : value =
  in_list in_tparam (in_typ_var "tparam" $ no_region) tparams
...
and in_params (params : P4.param list) : value =
  in_list in_param (in_typ_var "param" $ no_region) params
...
and in_cparams (cparams : P4.cparam list) : value =
  in_list in_cparam (in_typ_var "cparam" $ no_region) cparams

(* Type arguments *)

and in_targ (targ : P4.targ) : value =
  let mixop, values =
    match targ.it with
    | VoidT ->
        let mixop = [ [ in_atom "VoidT" ] ] in
    ...
    | TupleT typs ->
        let mixop = [ [ in_atom "TupleT" ]; [] ] in
        let value_typs = in_targs typs in
        (mixop, [ value_typs ])
    | NameT var ->
        let mixop = [ [ in_atom "NameT" ]; [] ] in
        let value_var = in_var var in
...
and in_targs (targs : P4.targ list) : value =
  in_list in_targ (in_typ_var "targ" $ no_region) targs
...
and in_args (args : P4.arg list) : value =
  in_list in_arg (in_typ_var "arg" $ no_region) args
...
and in_expr (expr : P4.expr) : value =
  let mixop, values =
    ...
    | BoolE { boolean } ->
        ...
        let value_boolean = in_bool boolean in
        (mixop, [ value_boolean ])
    | StrE { text } ->
        let mixop = [ [ in_atom "StrE" ]; [] ] in
        let value_text = in_text text in
        (mixop, [ value_text ])
    | NumE { num } ->
        let mixop = [ [ in_atom "NumE" ]; [] ] in
        let value_num = in_num num in
        (mixop, [ value_num ])
    | VarE { var } ->
    ...
    | SeqE { exprs } ->
        ...
        let value_exprs = in_exprs exprs in
        (mixop, [ value_exprs ])
    | SeqDefaultE { exprs } ->
    ...
    | RecordE { fields } ->
        ...
        let value_fields =
          ...
          let typ_pair = TupleT [ typ_member; typ_expr ] $ no_region in
          in_list
    ...
    | UnE { unop; expr } ->
        ...
        let value_unop = in_unop unop in
        let value_expr = in_expr expr in
        (mixop, [ value_unop; value_expr ])
    | BinE { binop; expr_l; expr_r } ->
        let mixop = [ [ in_atom "BinE" ]; []; []; [] ] in
        let value_binop = in_binop binop in
        let value_expr_l = in_expr expr_l in
        let value_expr_r = in_expr expr_r in
        (mixop, [ value_binop; value_expr_l; value_expr_r ])
    | TernE { expr_cond; expr_then; expr_else } ->
        let mixop = [ [ in_atom "TernE" ]; []; []; [] ] in
        let value_expr_cond = in_expr expr_cond in
        let value_expr_then = in_expr expr_then in
        let value_expr_else = in_expr expr_else in
    ...
    | TypeAccE { var_base; member } ->
        ...
        let value_var_base = in_var var_base in
        let value_member = in_member member in
    ...
    | CallFuncE { var_func; targs; args } ->
        ...
        let value_var_func = in_var var_func in
        let value_targs = in_targs targs in
    ...
    | CallTypeE { var_typ; member; targs; args } ->
        ...
        let value_var_typ = in_var var_typ in
        let value_member = in_member member in
        let value_targs = in_targs targs in
        let value_args = in_args args in
    ...
    | InstE { var_inst; targs; args } ->
        ...
        let value_var_inst = in_var var_inst in
        let value_targs = in_targs targs in
        let value_args = in_args args in
...
and in_exprs (exprs : P4.expr list) : value =
  in_list in_expr (in_typ_var "expr" $ no_region) exprs
...
and in_keysets (keysets : P4.keyset list) : value =
  in_list in_keyset (in_typ_var "keyset" $ no_region) keysets
...
and in_select_cases (select_cases : P4.select_case list) : value =
  in_list in_select_case (in_typ_var "selectcase" $ no_region) select_cases
...
and in_stmt (stmt : P4.stmt) : value =
  let mixop, values =
    ...
    | SwitchS { expr_switch; cases } ->
        ...
        let value_expr_switch = in_expr expr_switch in
        let value_cases = in_switch_cases cases in
        (mixop, [ value_expr_switch; value_cases ])
    | IfS { expr_cond; stmt_then; stmt_else } ->
        let mixop = [ [ in_atom "IfS" ]; []; []; [] ] in
        let value_expr_cond = in_expr expr_cond in
        let value_stmt_then = in_stmt stmt_then in
        let value_stmt_else = in_stmt stmt_else in
    ...
    | RetS { expr_ret } ->
        ...
        let value_expr_ret =
          in_opt in_expr (in_typ_var "expr" $ no_region) expr_ret
        in
    ...
    | TransS { expr_label } ->
        ...
        let value_expr_label = in_expr expr_label in
        (mixop, [ value_expr_label ])
    | DeclS { decl } ->
        let mixop = [ [ in_atom "DeclS" ]; [] ] in
        let value_decl = in_decl decl in
        (mixop, [ value_decl ])
  in
  let value =
    let vid = Value.fresh () in
...
and in_stmts (stmts : P4.stmt list) : value =
  in_list in_stmt (in_typ_var "stmt" $ no_region) stmts
...
and in_switch_case (switch_case : P4.switch_case) : value =
  let mixop, values =
    ...
    | MatchC (switch_label, block) ->
        ...
        let value_switch_label = in_switch_label switch_label in
        let value_block = in_block block in
...
and in_switch_cases (switch_cases : P4.switch_case list) : value =
  in_list in_switch_case (in_typ_var "switchcase" $ no_region) switch_cases
...
and in_decl (decl : P4.decl) : value =
  let mixop, values =
    ...
    | ConstD { id; typ; value; _ } ->
        ...
        let value_id = in_id id in
        let value_typ = in_typ typ in
        let value_value = in_expr value in
        (mixop, [ value_id; value_typ; value_value ])
    | VarD { id; typ; init; _ } ->
        let mixop = [ [ in_atom "VarD" ]; []; []; [] ] in
        let value_id = in_id id in
        let value_typ = in_typ typ in
        let value_init = in_opt in_expr (in_typ_var "expr" $ no_region) init in
        (mixop, [ value_id; value_typ; value_init ])
    | ErrD { members } ->
        let mixop = [ [ in_atom "ErrD" ]; [] ] in
        let value_members = in_members members in
        (mixop, [ value_members ])
    | MatchKindD { members } ->
    ...
    | StructD { id; tparams; fields; _ } ->
        ...
        let value_fields =
          ...
          let typ_pair = TupleT [ typ_member; typ_typ ] $ no_region in
          in_list (in_pair in_member in_typ typ_member typ_typ) typ_pair fields
        in
    ...
    | NewTypeD { id; typdef; _ } ->
        ...
        let value_id = in_id id in
        let value_typdef = in_typdef typdef in
    ...
    | TableD { id; table; _ } ->
        ...
        let value_id = in_id id in
        let value_table = in_table table in
    ...
    | ControlD { id; params; cparams; locals; body; _ } ->
        ...
        let value_cparams = in_cparams cparams in
        let value_locals = in_decls locals in
        let value_body = in_block body in
        ( mixop,
    ...
    | FuncD { id; typ_ret; tparams; params; body; _ } ->
        ...
        let value_id = in_id id in
        let value_typ_ret = in_typ typ_ret in
        let value_tparams = in_tparams tparams in
        let value_params = in_params params in
        let value_body = in_block body in
        ( mixop,
    ...
    | ExternObjectD { id; tparams; mthds; _ } ->
        ...
        let value_id = in_id id in
        let value_tparams = in_tparams tparams in
        let value_mthds = in_mthds mthds in
...
and in_decls (decls : P4.decl list) : value =
  in_list in_decl (in_typ_var "decl" $ no_region) decls
...
and in_parser_states (parser_states : P4.parser_state list) : value =
  in_list in_parser_state (in_typ_var "parserstate" $ no_region) parser_states

(* Tables *)

and in_table (table : P4.table) : value =
  in_list in_table_property (in_typ_var "tblprop" $ no_region) table
...
and in_table_keys (table_keys : P4.table_keys) : value =
  in_list in_table_key (in_typ_var "tblkey" $ no_region) table_keys.it
...
and in_table_actions (table_actions : P4.table_actions) : value =
  in_list in_table_action (in_typ_var "tblaction" $ no_region) table_actions.it
...
and in_table_entry (table_entry : P4.table_entry) : value =
  ...
  let mixop = [ []; []; []; []; [] ] in
  let value_table_entry_const = in_bool table_entry_const in
  let value_keysets = in_keysets keysets in
...
and in_table_entries (table_entries : P4.table_entries) : value =
  ...
  let mixop = [ []; []; [] ] in
  let value_table_entries_const = in_bool table_entries_const in
...
and in_table_custom (table_custom : P4.table_custom) : value =
  ...
  let mixop = [ []; []; []; [] ] in
  let value_table_custom_const = in_bool table_custom_const in
  let value_member = in_member member in
  let value_expr = in_expr expr in
  let value =
    let vid = Value.fresh () in
...
and in_mthds (mthds : P4.mthd list) : value =
  in_list in_mthd (in_typ_var "method" $ no_region) mthds
...
let in_program (includes_p4 : string list) (filename_p4 : string) : value =
  try
    ...
    |> in_list in_decl (in_typ_var "decl" $ no_region)
