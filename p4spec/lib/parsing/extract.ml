...
| _ -> ( 
      failwith (Printf.sprintf "Invalid name structure %s: %s " (Il.Print.string_of_value value) (id_of_case_v value))
    )
  ...
  let name_of_control_declaration (v: value) : string =
    ...
    | _ -> (
        ...
        failwith (Format.asprintf "invalid controlDeclaration structure: %a" Pp.pp_default_case_v v)
    )
  ...
  let name_of_any_declaration (v: value) : string =
    ...
    | _ -> failwith (Printf.sprintf "@name_of_any_declaration: Unknown declaration type: %s" (id_of_case_v v))
  ...
  let has_typ_params_declaration (_value_decl: value) : bool =
    false
